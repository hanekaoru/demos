...
```keys()``` 可以方便地返回一个 ```object``` 自身所有的 ```key```，但**不包含**从原型链继承下来的：
...
```allkeys()``` 除了 ```object``` 自身的 ```key```，还**包含**从原型链继承下来的：
...
和 ```keys()``` 类似，```values()``` 返回 ```object``` 自身但**不包含**原型链继承的所有值（注意：没有 ```allvalues()```）
...
如果我们要复制一个 ```object``` 对象，可以使用 ```clone()``` 方法，它会把原有对象的所有属性都赋值到新的对象中

但是需要需要的是，```clone()``` 是 "浅复制"，即两个对象相同的 ```key``` 所引用的 ```value``` 其实是同一对象
